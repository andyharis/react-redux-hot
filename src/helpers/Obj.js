...
module.exports.set = function (obj, prop, value) {
  if (typeof prop === 'number')
    prop = propToArray(prop.toString())
  else if (typeof prop === 'string')
    prop = propToArray(prop);
  console.info('%cChain is : %O','color:blue',prop);
  var setPropImmutableRec = function (obj, prop, value, i) {
    ...
    if (prop.length > i) {
      ...
      var emptyObject = {};
      if (typeof head === 'string' && head.match(/\[/)) {
        head = head.replace(/[\[\]]/g, '');
        emptyObject = obj[head] || [];
      }
      // if head is $add and parent obj is not an array
      // we're definitely know that this is creating
      // se we can create a new empty array with index 0
      if (head === '$add' && !Array.isArray(obj)) {
        // emptyObject = clone || [];
        clone = [];
        head = 0;//getArrayIndex(head,obj);
      }
      clone[head] = setPropImmutableRec((obj[head] !== undefined && obj[head] !== null) ? obj[head] : emptyObject, prop, value, i + 1);
      return clone;
    }
...
function getArrayIndex(head, obj) {
  ...
  if (!/^\+?\d+$/.test(head)) {
    throw new Error('You are trying to add object property to array! Array index \'' + head + '\' has to be an integer');
  }
